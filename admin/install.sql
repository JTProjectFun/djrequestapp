use djrequests;
CREATE TABLE IF NOT EXISTS systemUser(id int NOT NULL AUTO_INCREMENT, timedate varchar(32), username varchar(32) NOT NULL UNIQUE, password varchar(32), name varchar(32), email varchar(32), userlevel varchar(32), enabled int DEFAULT 1, PRIMARY KEY (id));
...
CREATE TABLE IF NOT EXISTS requestkeys (id int NOT NULL AUTO_INCREMENT, timedate varchar(32),thekey varchar(16) NOT NULL UNIQUE, date varchar(32), showrequests int(1), willexpire int(1), maxRequests int default 0, maxUserRequests int default 0, userid int, PRIMARY KEY(id));
...
("tooshort","There was a problem with the %FIELD%. Please try again."),
